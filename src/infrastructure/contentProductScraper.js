/**
 * DreamShop Content Product Scraper (True Monolith)
 * 
 * This function contains ALL site-specific logic and utilities internally
 * to ensure it can be injected into any page via chrome.scripting.executeScript
 * without external dependencies or import failures.
 */
export function scrapeProductFromPage(targetSelector = null) {
  // --- Inner Context: Site-Specific Handlers ---
    ...
    scrape: (root) => {
      ...
      if (state?.parsed) {
        ...
        return {
          ...
          currency: window.Shopify?.currency?.active || "USD", vendor: p.vendor, category: p.type,
          sku: firstVariant.sku || String(p.id), image_url: cleanImageUrl(p.featured_image || p.images?.[0] || ""),
      ...
      if (getMeta("shopify-product-id") || document.querySelector('meta[content*="shopify"]')) {
         return {
           title: getMeta("og:title") || document.title, price: getMeta("product:price:amount") || getMeta("og:price:amount"),
           ...
           image_url: cleanImageUrl(getMeta("og:image") || ""), extraction_method: "Expert-Shopify-Meta"
         };
      }
      return null;
    }
  ...
  const genericScraper = {
    findJsonLdProducts: () => {
      ...
      return scripts.flatMap(s => { try { const p = JSON.parse(s.textContent || ""); return Array.isArray(p) ? p : [p]; } catch (e) { return []; } })
        .flatMap(expandJsonLdNode).filter(n => (n["@type"] && String(n["@type"]).toLowerCase().includes("product")) || n.offers)
  ...
  function textFromSelectors(selectors, r = document) {
    for (const s of selectors) {
      const el = querySelectorDeep(s, r);
  ...
  function attributeValues(selectors, attrs, r = document) {
    const attrList = Array.isArray(attrs) ? attrs : [attrs];
    return selectors.flatMap(s => querySelectorAllDeep(s, r).map(el => {
  ...
  function firstObject(v) { const items = Array.isArray(v) ? v : [v]; return (items[0] && typeof items[0] === "object") ? items[0] : {}; }
  ...
  function scrapeBreadcrumbs(r = document) {
  ...
  function scrapeSpecifications(r = document) {
    const specs = {};
    ...
    selectors.forEach(s => {
      querySelectorAllDeep(s, r).forEach(el => {
  ...
  function scrapeWeight(r = document) {
  ...
  function collectAliExpressData(stateRoot) {
    const merged = {};
  ...
  function expandJsonLdNode(node) {
    ...
    if (node.offers) { const offers = Array.isArray(node.offers) ? node.offers : [node.offers]; nodes.push(...offers.flatMap(expandJsonLdNode)); }
    if (node.hasVariant) { const variants = Array.isArray(node.hasVariant) ? node.hasVariant : [node.hasVariant]; nodes.push(...variants.flatMap(expandJsonLdNode)); }
    return nodes;
  }
  ...
  function normalizeJsonLdProduct(data) {
    ...
    const imageValues = images.map((img) => (typeof img === "string" ? img : img?.url || img?.contentUrl || ""));
    return {
  ...
  const rootElement = targetSelector ? (document.querySelector(targetSelector) || document) : document;

  function scrapeDomProduct() {
    const title = textFromSelectors(["#productTitle", ".product-title", "h1.title", ".product-name h1", ".pdp-title", "h1"], rootElement);
    const price = textFromSelectors([".price", ".product-price", ".current-price", "[itemprop=\"price\"]", ".pdp-price", ".price-wrapper"], rootElement);
    const images = attributeValues(["meta[property=\"og:image\"]", "#landingImage", ".product-image img", "[data-testid=\"main-image\"] img"], ["content", "src", "data-src"], rootElement);
    ...
    let expertData = {};
    try {
      expertData = scraper ? (scraper.scrape(rootElement) || {}) : {};
    } catch (e) {
    ...
    return mergeProductData(
      expertData,
      {
        ...
        category: scrapeBreadcrumbs(rootElement),
        specifications: scrapeSpecifications(rootElement),
        variant_grams: scrapeWeight(rootElement),
  ...
  if (products.length === 0) {
    const title = querySelectorDeep("h1", rootElement)?.textContent?.trim() || document.title.split("-")[0].split("|")[0].trim();
    if (title) {
      products = [{
        title,
        image_url: cleanImageUrl(querySelectorDeep("img", rootElement)?.src || ""),
